...
class NewLandlordController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if(Auth::guard(userGuard())->check()){
            ...
            ->groupBy('users.id', 'birthday_email_reminders.status')
            ...
            $title = "Meesages";
            return view('newLandlord.index',compact('templates', 'title'));
        }
    }
    ...
    public function downloadContact(){
      ...
      $user_id = auth()->guard(userGuard())->user()->id;
    $users = User::where('role', 3)->where('status', 1)->whereRaw("FIND_IN_SET(?, users.agent_id)", [$user_id])->get(['name', 'phone_number']);
    ...
    $csvFileName = 'landlord_contacts_' . now()->format('YmdHis') . '.csv';
