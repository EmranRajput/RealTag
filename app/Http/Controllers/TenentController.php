...
class TenentController extends Controller
{
    ...
    public function index(){
		...
		return view($this->crudView('index'), [
			...
			'table' => 'tableTanant',
			// 'urlListData' => routePut('tanant.index'),
			// 'urlAdd' => routePut('tanant.add'),
    ...
    public function loadList()
	{
		try {
            ...
            $q = $q->where('role',User::ROLE_TENENT);
			$count = $q->count();
			...
			foreach ($q->get() as $single) {
				$data[] = [
                    ...
                    'activity'=>putNA(DataTableHelper::listActivity([
                        // 'report'=>routePut('tanant.report',['id'=>encId($single->id)]),
                        // 'sale'=>routePut('tanant.sales',['id'=>encId($single->id)]),
                    ])),
					'created_at' => putNA($single->showCreated(1)),
					'actions' => putNA(DataTableHelper::listActions([
						// 'edit' => routePut('tanent.report',['id'=>encId($single->id)]),
						// 'delete' => routePut('tanent.sales',['id'=>encId($single->id)]),
					]))
				];
			}
			return $this->resp(1, '', [
