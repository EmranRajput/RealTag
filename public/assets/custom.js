...
function _loader(action, msg) {
  let $div = jQuery(".page-loader-wrapper");
  if (action) $div.show();
  else $div.fadeOut();
...
function applyAdmissionDataTable(tableId, url, params, searchFun = {}) {
  var searchData = "";
  if (searchFun && searchFun != "") {
    searchData = searchFun;
  } else {
...
function formValidate(id, params) {
  ...
  params.errorPlacement = function (error, element) {
    if (jQuery(element).parents(".form-group").length) {
      ...
      jQuery(element).parents(".input-group").append(error);
    }
  };
...
function applyTogglePassword(eyeId, fieldId) {
