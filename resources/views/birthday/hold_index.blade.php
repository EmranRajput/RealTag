        ...
        <div class="row clearfix">
            <div class="col-md-12 p-l-0 p-r-0">
                <section class="boxs-simple">
                    <div class="d-flex">
                        <h4>Birthday Templates</h4>
                        <a href="{{ route('birthday.templates.edit')}}" class="create-new">
                            ...
                            <tr class="delete_{{$template->id}}">
                                ...
                                <td>
                                    <button class="deleteButton">
                                        <a href="{{ route('birthday.templates.edit',$template->id)}}">
...
$(".deleteBirthday").click(function() {
    ...
    var _url = "{{ route('birthday.template.delete')}}";
...
$(document).on('click', '.activate', function() {
    ...
    var _url = "{{ route('birthday.template.active.update.deactive')}}";
    $.ajax({
        type: "POST",
        url: _url,
        data: {
        ...
        success: function(response) {
            if (response.success) {
                if (response.status == 1) {
                    showAlert(1, "Birthday Template is activated");
                    $('.is_active_' + id).html(
                        '<button class="btn btn-success activate" data-id="' +
                        id +
                        '"> Active</button>');
                } else if (response.status == 0) {
                    showAlert(1, "Birthday Template is deactivated");
                    $('.is_active_' + id).html(
                ...
                showAlert(0, "Birthday Template is deactivate");
            }
        }
    });
});
...
function handleRadioClick(data) {
    ...
    var _url = "{{ route('birthday.template.set.default')}}";
    $.ajax({
        type: "POST",
        url: _url,
        data: {
