        ...
        <div class="row clearfix">
            <div class="col-md-12 p-l-0 p-r-0">
                <section class="boxs-simple">
                    <div class="d-flex">
                        <h4>Landlords</h4>
                        <a href="{{ route('agent.landlord.downloadContact')}}" class="create-new" style="margin-right: -50%;">
                            <button class="btn btn-warning  mb-2" style="color:#ffffff"> <i class="fa fa-file-csv"></i> Export Contact</button></a>
                        <a href="{{ route('agent.landlord.edit')}}" class="create-new">
                            <button class="btn btn-primary  mb-2"><i class="fa fa-plus"></i>  Create New</button></a>
                        <form action="{{ route('agent.landlord') }}" method="GET" class="row">
                            ...
                            <tr class="delete_{{$template->id}}">
                                ...
                                <td>
                                    <button class="deleteButton">
                                        <a href="{{ route('agent.landlord.edit',$template->id)}}">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        showAlert(1, "Landlord is removed");
...
$(document).on('click', '.activate', function() {
    ...
    var _url = "{{ route('agent.landlord.active.update.deactive')}}";
    $.ajax({
        type: "POST",
        url: _url,
        data: {
        ...
        success: function(response) {
            if (response.success) {
                ...
                showAlert(0, "Landlord is deactivate");
            }
        }
    });
});
